...
export const RecipeList = (props: RecipeComponentProps) => {
  ...
  return (
    <ol className={classes.recipe_steps_list}>
      {
        recipe && recipe.steps.map((step:Step, i:number) => {
          return (
            <li className={classnames(classes.recipe_steps_list_step, highlightedStep === (i+1) ? classes.recipe_step_highlight : '')} id={`steps-list-${i}`} key={`step-${i}`}>
            <b className={classes.recipe_steps_list_verb}>{step.verb} </b>
            {
              step.ingredients.map((ing) => {
                ...
                if (stepRef) {
                  ...
                  return <span
                    onMouseOver={() => { handleStepMouseOver(stepRef) }}
                    onMouseOut={handleStepMouseOut}
                    className={classnames(classes.recipe_steps_list_ingredient, classes.recipe_steps_list_ingredient_ref)}
                    key={ing}>{ing}</span>
                } else {
                  return <span className={classes.recipe_steps_list_ingredient} key={ing}>{ing}</span>
                }
              })
            }
          </li>)
